...
public class PokeMove extends Thread {
    ...
    public void runPokestop() throws InterruptedException, LoginFailedException, RemoteServerException {
        while (true) {
            ...
            for (Pokestop p : lesPokestops) {
                if (p.canLoot()) {
                    ...
                    if (p.canLoot(true)) {
                        double distanceTmp = distance(p.getLatitude(), p.getLongitude(), go.getLatitude(), go.getLongitude());
                        if (nearest == null) {
                            nearest = p;
            ...
            attendre(lesPokestopALeure);
            moveTo(nearest);
        }
    }

    public void attendre(List<Pokestop> lesPokestopALeure) throws LoginFailedException, RemoteServerException, InterruptedException {
        while (lesPokestopALeure.size() > 0) {
            ...
            if (lesPokestopALeure.size() > 0 && minTime != Long.MAX_VALUE) {
                logger.info("je vais attendre :  " + (Math.max(minTime - System.currentTimeMillis(), 60000) + 1000) / 1000 + " secondes, il y a toujours un leurre ici");
                Thread.sleep(Math.max(minTime - System.currentTimeMillis(), 60000) + 1000);
            }
        }
    }
    ...
    public void moveTo(Pokestop p) throws LoginFailedException, RemoteServerException {
        ...
        logger.info("aller au pokestop  : " + p.getDetails().getName() + " : " + (int) distance(go.getLatitude(), p.getLongitude(), p.getLatitude(), p.getLongitude()) + " metres");
